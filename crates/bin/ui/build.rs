...
fn main() {
    ...
    shader_compiler
        .compile_shader("shaders/ui.vert", ShaderStage::Vertex)
        .expect("Could not compile shader");
    shader_compiler
        .compile_shader("shaders/ui.frag", ShaderStage::Fragment)
        .expect("Could not compile shader");
    // TODO: rkvy stuff should be optional
    write_shader_structs(&std::fs::read("./shaders/spv/ui.vert.spv").unwrap(), "ui");
}
