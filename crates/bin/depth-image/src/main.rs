...
impl Renderer {
    pub fn new(window: &winit::window::Window) -> Result<Self> {
        ...
        let mut vertex_shader = device.create_shader(
            include_bytes!("../shaders/spv/depth.vert.spv"),
            Default::default(),
        )?;
        let mut fragment_shader = device.create_shader(
            include_bytes!("../shaders/spv/depth.frag.spv"),
            Default::default(),
        )?;
        ...
        let ubo_buffer = device.create_buffer(
            std::mem::size_of::<DepthUniformBufferObject>() as u64,
            BufferDescription {
        ...
        ubo_buffer.mem_copy(
            util::offset_of!(DepthUniformBufferObject, view) as u64,
            &[
                look_to(
    ...
    pub fn update(&mut self) -> Result<()> {
        ...
        self.ubo_buffer.mem_copy(
            util::offset_of!(DepthUniformBufferObject, model) as u64,
