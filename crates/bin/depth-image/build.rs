...
fn main() {
    ...
    shader_compiler
        .compile_shader("shaders/depth.vert", ShaderStage::Vertex)
        .expect("Could not compile shader");
    shader_compiler
        .compile_shader("shaders/depth.frag", ShaderStage::Fragment)
        .expect("Could not compile shader");
    // TODO: rkvy stuff should be optional
    write_shader_structs(
        &std::fs::read("./shaders/spv/depth.vert.spv").unwrap(),
        "depth",
    );
}
