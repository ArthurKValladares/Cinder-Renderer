...
include!(concat!(
    ...
    "/gen/frame_graph_shader_structs.rs"
));
...
impl Vertex for FrameGraphVertex {
...
impl Renderer {
    pub fn new(window: &winit::window::Window) -> Result<Self> {
        ...
        let vertex_shader = device.create_shader(
            include_bytes!("../shaders/spv/frame_graph.vert.spv"),
            Default::default(),
        )?;
        let fragment_shader = device.create_shader(
            include_bytes!("../shaders/spv/frame_graph.frag.spv"),
            Default::default(),
        )?;
        ...
        let scene = Scene::<FrameGraphVertex>::from_obj(
        ...
        let ubo_buffer = device.create_buffer(
            std::mem::size_of::<FrameGraphUniformBufferObject>() as u64,
            BufferDescription {
        ...
        ubo_buffer.mem_copy(
            util::offset_of!(FrameGraphUniformBufferObject, view) as u64,
            &[
                look_to(
    ...
    pub fn update(&mut self) -> Result<()> {
        ...
        self.ubo_buffer.mem_copy(
            util::offset_of!(FrameGraphUniformBufferObject, model) as u64,
            &[
