...
include!(concat!(
    ...
    "/gen/blur_mesh_shader_structs.rs"
));
include!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/gen/blur_texture_shader_structs.rs"
));
...
impl Renderer {
    pub fn new(window: &winit::window::Window) -> Result<Self> {
        ...
        let blur_image = device.create_image(
        ...
        let mesh_render_pipeline = {
            let mut vertex_shader = device.create_shader(
                include_bytes!("../shaders/spv/blur_mesh.vert.spv"),
                Default::default(),
            )?;
            let mut fragment_shader = device.create_shader(
                include_bytes!("../shaders/spv/blur_mesh.frag.spv"),
                Default::default(),
            )?;
        ...
        let texture_render_pipeline = {
            let mut vertex_shader = device.create_shader(
                include_bytes!("../shaders/spv/blur_texture.vert.spv"),
                Default::default(),
            )?;
            let mut fragment_shader = device.create_shader(
                include_bytes!("../shaders/spv/blur_texture.frag.spv"),
                Default::default(),
            )?;
            let render_pipeline = device.create_graphics_pipeline(
                &vertex_shader,
        ...
        let ubo_buffer = device.create_buffer(
            std::mem::size_of::<BlurMeshUniformBufferObject>() as u64,
            BufferDescription {
        ...
        ubo_buffer.mem_copy(
            util::offset_of!(BlurMeshUniformBufferObject, view) as u64,
            &[
                look_to(
        ...
        {
            upload_context.transition_depth_to_read_only(&device, &blur_image);
        }
        upload_context.end(
            &device,
            device.setup_fence(),
        ...
        device.write_bind_group(
            ...
            &[BindGroupBindInfo {
                dst_binding: 0,
                data: BindGroupWriteData::SampledImage(blur_image.bind_info(
                    &sampler,
    ...
    pub fn update(&mut self) -> Result<()> {
        ...
        self.ubo_buffer.mem_copy(
            util::offset_of!(BlurMeshUniformBufferObject, model) as u64,
    ...
    pub fn resize(&mut self, width: u32, height: u32) -> Result<()> {
        ...
        {
            self.upload_context
                .transition_depth_to_read_only(&self.device, &self.blur_image);
        }
        ...
        self.device.write_bind_group(
            ...
            &[BindGroupBindInfo {
                dst_binding: 0,
                data: BindGroupWriteData::SampledImage(self.blur_image.bind_info(
                    &self.sampler,
                    Layout::DepthStencilReadOnly,
                    0,
                )),
...
impl Drop for Renderer {
    fn drop(&mut self) {
        ...
        self.blur_image.destroy(self.device.raw());
