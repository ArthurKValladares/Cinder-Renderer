...
fn main() {
    ...
    shader_compiler
        .compile_shader("shaders/blur_mesh.vert", ShaderStage::Vertex)
        .expect("Could not compile shader");
    shader_compiler
        .compile_shader("shaders/blur_mesh.frag", ShaderStage::Fragment)
        .expect("Could not compile shader");
    write_shader_structs(
        &std::fs::read("./shaders/spv/blur_mesh.vert.spv").unwrap(),
        "blur_mesh",
    );

    shader_compiler
        .compile_shader("shaders/blur_texture.vert", ShaderStage::Vertex)
        .expect("Could not compile shader");
    shader_compiler
        .compile_shader("shaders/blur_texture.frag", ShaderStage::Fragment)
        .expect("Could not compile shader");
    write_shader_structs(
        &std::fs::read("./shaders/spv/blur_texture.vert.spv").unwrap(),
        "blur_texture",
    );
}
